...
def create_monthly_data_task(self, asset_class: str, year: int, month: int, symbol: str, exchange: str, request_id: str = None):
    logger.info("Celery task start: %s", request_id)
    try:
        return asyncio.run(create_monthly_data(asset_class, year, month, symbol, exchange))
